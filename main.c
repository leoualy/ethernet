...
#define DES_MAC {0xb8,0x86,0x87,0x49,0x92,0xbf}
#define SRC_IP "192.168.2.1"
#define DES_IP "192.168.2.80"
...
void fill_eth_hdr(unsigned char *src_mac,unsigned char *des_mac){
...
int main(){
    ...
    fill_eth_hdr(src_mac,des_mac);
    fill_arp(src_mac,des_mac,(char *)SRC_IP,(char *)DES_IP);
    while(1){
        int retlen = sendto(fd,buf,ETH_PACKET_LEN,0,(struct sockaddr *)&saddr_ll,sizeof(struct sockaddr_ll));
        printf("retlen is %d\n",retlen);
    }
